  ...
  protected_settings   = <<SETTINGS
    {
        ...
        "commandToExecute": "powershell.exe -ExecutionPolicy Unrestricted -File agentinstall.ps1 -account \"${var.VSTS_ACCOUNT}\" -PAT \"${var.VSTS_TOKEN}\" -PoolNamePrefix \"${var.VSTS_POOL_PREFIX}\" -ComputerName \"${var.vm_name}-${var.run_date}\" -AdminAccount \"${var.ADMIN_USERNAME}\" -AdminPassword \"${var.ADMIN_PASSWORD}\" -count \"${var.VSTS_AGENT_COUNT}\" -PartitionSize \"${local.diskSize}\""
    }
SETTINGS
}
